            ...
            if task.isdigit():
                if task == '3':
                    ...
                    if phone == '0':
                        ...
                        Thread(target=Bomber(phone).call, daemon=True).start()
                        phones_in_spam.append(phone)
                        print(f'Спам на номер {phone} начат.')
                        # сюда таймер и блок при шотдувне
                    ...
                    if phone == '0':
                        ...
                        phone = phone_format(phone)
                        tR=Thread(target=Bomber(phone).rus, daemon=True).start()
                        Thread(target=check_time(tR,inn),daemon=True).start()
                        phones_in_spam.append(phone)
